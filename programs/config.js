// Coach Bertin V50.18 — configuration programme extraite de app.js
...
var movements = {
  ...
  // V50.18 : aucun Chest Supported Row dans l'app. Alias conservé pour ne pas casser les anciennes données.
...
// V50.18 propre — correctifs runtime centralisés dans CE fichier seulement.
(function coachBeurtV5018RuntimePatch(){
  ...
  function findAthleteMovement(name){
    ...
    var aliases=[wanted];
    if(wanted==="barbell row") aliases.push("barbell row","chest supported row","chest row","row principal");
    var keys=Object.keys(ast);
    for(var i=0;i<keys.length;i++){
      var k=norm(keys[i]);
      if(aliases.indexOf(k)>=0)return ast[keys[i]];
    }
    return null;
  }
  ...
  function findAutoLoadHint(name){
    ...
    for(var i=0;i<keys.length;i++){
      if(key&&keys[i]&&(key.indexOf(keys[i])>=0||keys[i].indexOf(key)>=0))return map[keys[i]];
    }
    return null;
  }
  ...
  later(function(){
    try{
      ...
      window.athleteSuggestedLoad=function(nameOrKey,currentLoad,targetReps){ return smartSuggestedLoad(nameOrKey,currentLoad,targetReps); };
      ...
      window.showLoadInfoModal=function(msg){
        ...
        if(hint){
          var rows=(hint.rows&&hint.rows.length)?hint.rows:[{date:"—",load:"—",reps:"—",rpe:"—"}];
          var lis=rows.map(function(r){
            return "<li>"+escapeHtml((r.date||"?")+" — "+(r.load||"?")+" lb × "+(r.reps||"?")+" — RPE "+(r.rpe||"?"))+"</li>";
          }).join("");
          modal.innerHTML =
            '<div class="tuto-modal-inner">'+
              ...
              '<div class="tuto-section"><div class="tuto-section-title">Dernières séances</div><ul>'+lis+'</ul></div>'+
    ...
    }catch(e){ console.warn("Coach Beurt V50.18 patch non appliqué",e); }
  });
})();
