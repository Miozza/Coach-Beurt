...
async function readGithubJsonFile(token,path){
  ...
  try{
    ...
    if(window.CoachLog)CoachLog.error("github_read_failed", e, {path:path});
    return{ok:false,missing:false,msg:"Erreur réseau : "+e.message};
  }
}
async function writeGithubFile(token,path,text,message,sha){
  ...
  try{
    ...
    if(window.CoachLog)CoachLog.error("github_write_failed", e, {path:path});
...
async function testGithubToken(){
  ...
  try{
    ...
    if(window.CoachLog)CoachLog.error("github_token_test_failed", e, {});
...
async function syncHistoryFromGitHub(silent){
  ...
  try{
    ...
    if(window.CoachLog)CoachLog.error("github_sync_failed", e, {silent:!!silent});
...
async function savePrProfile(){
  ...
  if(!token){
    ...
    try{
      ...
      if(window.CoachLog)CoachLog.error("session_github_save_failed", e, {week:state.week,day:state.day});
