...
function updateWodTimerDisplay(){
  // V50.7 — timer WOD retiré de la vue iPhone. Aucun élément à mettre à jour.
}
...
function setupWodTimer(){
  ...
  updateWodTimerDisplay();
  var start=null,pause=null,reset=null; // V50.7 — timer WOD retiré
...
// V50.7 — horloge uniquement dans le mode séance; heure et secondes même grosseur.
...
function updateRestFloatingClock(){
  // V50.7 : l'heure permanente remplace l'ancienne horloge de repos.
...
function updateRestDisplay(){
  // V50.7 — restDisplay retiré du DOM. Seul updateRestFloatingClock est conservé.
...
function parseWodStructure(text){
  ...
  function cleanMoveName(name, isCal){
    ...
    // V50.7 : dans la vue séance, on doit voir "Cal Row" et non juste "Row".
  ...
  Object.keys(results||{}).forEach(function(key){
    ...
    if(!existing||load>=existing.load){
      state.movementRefs[refK]={
        ...
        rpe:Number(r.rpe)||8,note:"Saisi V50.7"
      };
    }
...
// V50.7 — volontairement neutralisé.
...
// V50.7 — supprimé/neutralisé : l'app ne doit jamais écrire charges.js automatiquement.
...
// ─── GitHub API helpers ─────────────────────────────────────────────────────
// V50.7 : helpers GitHub globaux. Sans elles, le test token/PR plante.
...
// ─── Mode séance guidé (optionnel) — V50.7 ────────────────────────────────
...
// V50.7 — Résultats saisis directement dans le mode séance.
...
// V50.7 — synchronisation immédiate mode séance → saisie iPhone.
...
// V50.7 — Vue séance : échauffement compact.
 // Les mouvements gardent leur nom complet, mais aucun but/objectif n’est affiché directement.
 // Le but reste seulement dans le tuto ? si l’utilisateur le demande.
...
function renderGuidedSession(){
  ...
  if(st.kind==="wod"){
    ...
    if(st.kind==="warmup" || st.kind==="mobility" || st.kind==="bonus"){
      ...
      // Avant V50.7, ils s'affichaient vides en mode séance.
